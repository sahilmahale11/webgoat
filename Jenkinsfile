    ...
    stage ('Incidents report') {
        steps {
	        ...
	        sh 'cd securitytoolsparser-main/ && python3 run_parser.py -t "DependencyCheck Scan" -p "../dependency-check-report.xml" -o "consolidated_dependency_check_output.csv"'
                sh 'cd securitytoolsparser-main/ && python3 run_parser.py -t "Sonarqube" --base_url "http://3.85.36.214:9000" --project_branch "main" --project_key "org.owasp.webgoat:webgoat" --user_token "squ_de715a18ac0c010239d5fbb220e42fca00c1a21f" -p "sonarqube_report.json" -o "consolidated_sonarqube_scan_output.csv"'
